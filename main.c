/*
 * ULTIMATE WINDOWS SYSTEM CONTROL MENU
 * Pure C / Win32 API - Fully Functional Windows Controls
 * Compatible with MinGW/GCC
 *
...
void OnTopAbout(void) { ShowInfo("Ultimate Windows Control Menu v1.0\n\nOver 150 functional commands\nFull Windows system integration\n\nWritten in pure C with Win32 API"); }
...
BOOL AddTrayIcon(HWND hWnd) {
    ...
    strcpy(g_nid.szTip, "Functional Menu - Right Click");
...
/* ----------------------------------------------------------------------------
 * EXTENSION: Additional Tools + Full English GUI Control Panel
 * Append this entire block to the end of the original main.c file.
 * No existing code is modified; only new definitions and functions are added.
 * ------------------------------------------------------------------------- */

// New base IDs (ensure they don't collide with existing ranges)
...
// ============================================================================
// Handler declarations (forward)
// ============================================================================
void OnAdvLusrmgr(void);
void OnAdvSecpol(void);
void OnAdvWF(void);
void OnAdvCertmgr(void);
void OnAdvOdbc(void);
void OnAdvComServices(void);
void OnAdvTpm(void);
void OnAdvBitlocker(void);
void OnAdvCredMgr(void);
void OnAdvAutoPlayCpl(void);
void OnAdvColorMgmt(void);

void OnSysCfgEnvVar(void);
void OnSysCfgPerfOpt(void);
void OnSysCfgUserProf(void);
void OnSysCfgStartRec(void);
void OnSysCfgHardwareProf(void);
void OnSysCfgRemote(void);
void OnSysCfgSysProtection(void);

void OnNetAdvNcpa(void);
void OnNetAdvSharingCenter(void);
void OnNetAdvSharing(void);
void OnNetAdvWifiSense(void);
void OnNetAdvNetReset(void);
void OnNetAdvFwRules(void);
void OnNetAdvInetProp(void);

void OnSecAdvDefender(void);
void OnSecAdvVirusThreat(void);
void OnSecAdvFirewallProt(void);
void OnSecAdvAppBrowser(void);
void OnSecAdvDeviceSec(void);
void OnSecAdvDevicePerf(void);
void OnSecAdvFamily(void);

void OnDevDiagDevEnv(void);
void OnDevDiagWsl(void);
void OnDevDiagWinTerminal(void);
void OnDevDiagPerfMon(void);
void OnDevDiagRelia(void);
void OnDevDiagStepsRec(void);
void OnDevDiagWpr(void);
void OnDevDiagGpedit(void);

void OnMorePen(void);
void OnMoreAutoplay(void);
void OnMorePhoneLink(void);
void OnMoreNearby(void);
void OnMoreClipboard(void);
void OnMoreFocusAssist(void);
void OnMoreProjecting(void);
void OnMoreTabletMode(void);

void OnAdvSysWinTools(void);
void OnAdvSysHyperV(void);
void OnAdvSysIscsi(void);
void OnAdvSysStorageSpaces(void);
void OnAdvSysCleanMgrAdv(void);
void OnAdvSysTaskmgrDetails(void);
void OnAdvSysResmon(void);
void OnAdvSysMsinfo32(void);

void OnGuiOpenPanel(void);

// ============================================================================
// Handler implementations (run Windows tools)
// ============================================================================
...
// ============================================================================
// Add new entries to the command dispatch table
// ============================================================================
// Append the following lines to the end of the g_CommandTable array.
// (We'll provide a separate function to append them at runtime, or user can manually insert.)
// Since we cannot modify the existing table definition, we will create a second
// table and chain the lookup in FindHandler.
// ============================================================================
static CommandEntry g_CommandTableExt[] = {
    // Advanced Tools
...
// Override FindHandler to search both tables
CommandHandler FindHandlerExt(UINT uID) {
    CommandHandler h = FindHandler(uID);
    if (h) return h;
    for (int i = 0; g_CommandTableExt[i].uID != 0; i++)
        if (g_CommandTableExt[i].uID == uID)
            return g_CommandTableExt[i].handler;
    return NULL;
}

// Macro to redirect original FindHandler calls to our extended version
#define FindHandler FindHandlerExt

// ============================================================================
// GUI Control Panel Implementation
// ============================================================================
#define IDC_BTN_SYSINFO      101
#define IDC_BTN_DEVMGMT      102
#define IDC_BTN_DISKMGMT     103
#define IDC_BTN_SERVICES     104
#define IDC_BTN_TASKSCHED    105
#define IDC_BTN_EVENTVWR     106
#define IDC_BTN_REGEDIT      107
#define IDC_BTN_MSINFO32     108
#define IDC_BTN_MS_CONFIG    109
#define IDC_BTN_TASKMGR      110
#define IDC_BTN_CMDPROMPT    111
#define IDC_BTN_POWERSHELL   112
#define IDC_BTN_SETTINGS     113
#define IDC_BTN_CONTROLPANEL 114
#define IDC_BTN_EXIT         115

HWND g_hGuiDlg = NULL;

INT_PTR CALLBACK GuiDlgProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        case WM_INITDIALOG:
            SetWindowText(hDlg, "Ultimate Windows Control Panel");
            // Set button texts
            SetDlgItemText(hDlg, IDC_BTN_SYSINFO, "System Properties");
            SetDlgItemText(hDlg, IDC_BTN_DEVMGMT, "Device Manager");
            SetDlgItemText(hDlg, IDC_BTN_DISKMGMT, "Disk Management");
            SetDlgItemText(hDlg, IDC_BTN_SERVICES, "Services");
            SetDlgItemText(hDlg, IDC_BTN_TASKSCHED, "Task Scheduler");
            SetDlgItemText(hDlg, IDC_BTN_EVENTVWR, "Event Viewer");
            SetDlgItemText(hDlg, IDC_BTN_REGEDIT, "Registry Editor");
            SetDlgItemText(hDlg, IDC_BTN_MSINFO32, "System Information");
            SetDlgItemText(hDlg, IDC_BTN_MS_CONFIG, "System Configuration");
            SetDlgItemText(hDlg, IDC_BTN_TASKMGR, "Task Manager");
            SetDlgItemText(hDlg, IDC_BTN_CMDPROMPT, "Command Prompt");
            SetDlgItemText(hDlg, IDC_BTN_POWERSHELL, "PowerShell");
            SetDlgItemText(hDlg, IDC_BTN_SETTINGS, "Settings");
            SetDlgItemText(hDlg, IDC_BTN_CONTROLPANEL, "Control Panel");
            SetDlgItemText(hDlg, IDC_BTN_EXIT, "Exit");
            return TRUE;

        case WM_COMMAND:
            switch (LOWORD(wParam)) {
                case IDC_BTN_SYSINFO:    OpenControlPanel("sysdm.cpl"); break;
                case IDC_BTN_DEVMGMT:    RunSystem("devmgmt.msc"); break;
                case IDC_BTN_DISKMGMT:   RunSystem("diskmgmt.msc"); break;
                case IDC_BTN_SERVICES:   RunSystem("services.msc"); break;
                case IDC_BTN_TASKSCHED:  RunSystem("taskschd.msc"); break;
                case IDC_BTN_EVENTVWR:   RunSystem("eventvwr.msc"); break;
                case IDC_BTN_REGEDIT:    RunSystem("regedit.exe"); break;
                case IDC_BTN_MSINFO32:   RunSystem("msinfo32.exe"); break;
                case IDC_BTN_MS_CONFIG:  RunSystem("msconfig.exe"); break;
                case IDC_BTN_TASKMGR:    RunSystem("taskmgr.exe"); break;
                case IDC_BTN_CMDPROMPT:  RunSystem("cmd.exe"); break;
                case IDC_BTN_POWERSHELL: RunSystem("powershell.exe"); break;
                case IDC_BTN_SETTINGS:   OpenSettings(""); break;
                case IDC_BTN_CONTROLPANEL: OpenControlPanel(""); break;
                case IDC_BTN_EXIT:
                case IDCANCEL:
                    DestroyWindow(hDlg);
                    g_hGuiDlg = NULL;
...
void OnGuiOpenPanel(void) {
    ...
    g_hGuiDlg = CreateDialog(g_hInst, MAKEINTRESOURCE(100), NULL, GuiDlgProc);
    ShowWindow(g_hGuiDlg, SW_SHOW);
}

// Resource script for the dialog (to be embedded manually or compiled with .rc)
// You can compile with: windres resource.rc -o resource.o && gcc ... resource.o
/*
// Save this as resource.rc
#include <windows.h>
100 DIALOGEX 0, 0, 380, 240
STYLE DS_SETFONT | DS_MODALFRAME | WS_POPUP | WS_CAPTION | WS_SYSMENU
CAPTION "Ultimate Windows Control Panel"
FONT 8, "MS Shell Dlg", 400, 0, 0x1
BEGIN
    PUSHBUTTON      "System Properties", IDC_BTN_SYSINFO, 10, 10, 110, 25
    PUSHBUTTON      "Device Manager", IDC_BTN_DEVMGMT, 130, 10, 110, 25
    PUSHBUTTON      "Disk Management", IDC_BTN_DISKMGMT, 250, 10, 110, 25
    PUSHBUTTON      "Services", IDC_BTN_SERVICES, 10, 40, 110, 25
    PUSHBUTTON      "Task Scheduler", IDC_BTN_TASKSCHED, 130, 40, 110, 25
    PUSHBUTTON      "Event Viewer", IDC_BTN_EVENTVWR, 250, 40, 110, 25
    PUSHBUTTON      "Registry Editor", IDC_BTN_REGEDIT, 10, 70, 110, 25
    PUSHBUTTON      "System Information", IDC_BTN_MSINFO32, 130, 70, 110, 25
    PUSHBUTTON      "System Configuration", IDC_BTN_MS_CONFIG, 250, 70, 110, 25
    PUSHBUTTON      "Task Manager", IDC_BTN_TASKMGR, 10, 100, 110, 25
    PUSHBUTTON      "Command Prompt", IDC_BTN_CMDPROMPT, 130, 100, 110, 25
    PUSHBUTTON      "PowerShell", IDC_BTN_POWERSHELL, 250, 100, 110, 25
    PUSHBUTTON      "Settings", IDC_BTN_SETTINGS, 10, 130, 110, 25
    PUSHBUTTON      "Control Panel", IDC_BTN_CONTROLPANEL, 130, 130, 110, 25
    PUSHBUTTON      "Exit", IDC_BTN_EXIT, 250, 130, 110, 25
END
*/

// ============================================================================
// Add new menus to ShowContextMenu (append this block inside ShowContextMenu)
// ============================================================================
...
// Modify ShowContextMenu to call AppendNewMenus right before TrackPopupMenu.
// Since we cannot edit the original function, we'll provide a wrapper or
// instruct the user to insert one line:
//    AppendNewMenus(hPopup);
// after all the original AppendMenu calls and before:
